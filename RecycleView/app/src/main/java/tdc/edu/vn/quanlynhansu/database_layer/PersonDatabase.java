...
public class PersonDatabase extends SQLiteOpenHelper {
    ...
    public void savePerson(Person person) {
        ...
        if (database != null) {
            if(!findPerson(person,database)){
                ...
                builder.setMessage("The person is exit and not inserted to in database");
    ...
    public void savePersons(ArrayList<Person> people) {
        ...
        if (database != null) {
            ...
            if(sum > 0){
                AlertDialog.Builder builder = new AlertDialog.Builder(context);
                builder.setTitle(context.getResources().getString(R.string.lblDialogTitle));
                builder.setMessage("There are + " + sum + " persons inserted into databse");
                builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
    ...
    public void updatePerson(Person oldPerson, Person newPerson) {
        ...
        if (database != null) {
            ...
            int updatedRow = database.update(PER_TABLE_NAME, values, where, whereArgs);
            // TODO Show dialog to user
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(context.getResources().getString(R.string.lblDialogTitle));
            if (updatedRow > 0) {
                builder.setMessage("There are/is" + updatedRow + " persons updated");
            } else {
                builder.setMessage("There are/is no persons updated");
            }
            builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
    ...
    public void deletePerson(Person person) {
        ...
        if (database != null) {
            ...
            int deleteRow = database.delete(PER_TABLE_NAME, where, whereArgs);
//            database.delete(PER_TABLE_NAME,where,whereArgs);
            // TODO Show dialog to user
            AlertDialog.Builder builder = new AlertDialog.Builder(context);
            builder.setTitle(context.getResources().getString(R.string.lblDialogTitle));
            if (deleteRow > 0) {
                builder.setMessage("There are/is" + deleteRow + " persons deleted");
            } else {
                builder.setMessage("There are/is no persons deleted");
            }
            builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
    ...
    private boolean findPerson(Person person, SQLiteDatabase database) {
        boolean ok = false;
        String[] selectionColunm = new String[]{PER_ID, PER_NAME, PER_DEGREE, PER_HOBBIES};
        String whereCondition = PER_NAME + " =? AND " + PER_DEGREE + " =? AND " + PER_HOBBIES + " =?";
